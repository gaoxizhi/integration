...
public class MappedFile {
    ...
    private long MAX_FLUSH_DATA_SIZE = 512 * 1024;
    ...
    public synchronized boolean appendData(byte[] data) throws Exception {
        ...
        // 检查是否需要把内存缓冲刷到磁盘
        boolean dataSize = writePosition - lastFlushFilePosition > this.MAX_FLUSH_DATA_SIZE;
        boolean timeCheck = System.currentTimeMillis() - lastFlushTime > this.MAX_FLUSH_TIME_GAP;
        boolean positionCheck = writePosition > lastFlushFilePosition;

        if (dataSize || timeCheck && positionCheck) {
            // 刷到磁盘
            flush();
        }
        return true;
