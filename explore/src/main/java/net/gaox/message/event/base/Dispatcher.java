...
/**
 * <p> 分发广播消息的分发器 </p>
 * Dispatcher 的主要作用是将 EventBus post 的 event 推送给每一个注册到 topic 上的 subscriber 上
 * 具体的推送其实就是执行 Subscribe注解的方法
 *
 * @author gaox·Eric
 * @date 2024-02-01 11:07
 */
    ...
    public void dispatch(Bus bus, Registry registry, Object event, String topic) {
        ...
        if (null == subscribers) {
            if (exceptionHandler != null) {
                IllegalArgumentException e = new IllegalArgumentException("The topic " + topic + " not bind yet");
                exceptionHandler.handle(e, new BaseEventContext(bus.getBusName(), null, event));
            }

            return;
        }
        // 遍历所有的方法，并且通过反射的方式进行方法调用
        // 同步的缺点，若其中的一个 subscribe 方法运行时间比较长，则会影响下一个 subscribe 方法的执行
        // 即使并行，效果也不是非常理想
        subscribers.stream().filter(subscriber -> !subscriber.isDisable())
                ...
                }).parallel().forEach(subscriber -> realInvokeSubscribe(subscriber, event, bus));
    }
