...
public class WriteBigFileComparison {
     ...
     */
    private static final long DATA_LENGTH = 2L * 1024 * 1024 * 1024L;


    public static void writeWithFileChannel() throws IOException {
        File file = new File("J:/lp/fc.dat");
        if (file.exists()) {
            file.delete();
        }
    ...
    public static void writeWithMappedByteBuffer() throws IOException {
        File file = new File("J:/lp/mb.dat");
        if (file.exists()) {
            file.delete();
        }
    ...
    public static void writeWithTransferTo() throws IOException {
        File file = new File("J:/lp/transfer.dat");
        if (file.exists()) {
            file.delete();
        }
        ...
        while (len >= DATA_CHUNK) {
            ...
            toFileChannel.transferFrom(fromByteChannel, position, (long) DATA_CHUNK);
        ...
        if (len > 0) {
            ...
            toFileChannel.transferFrom(fromByteChannel, position, len);
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        sw.start("write with file channel's transferTo");
        writeWithTransferTo();
        System.out.println(sw.prettyPrint());
        sw.stop();
