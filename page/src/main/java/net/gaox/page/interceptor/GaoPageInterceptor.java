...
public class GaoPageInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        while (handler.hasGetter("h")) {
            Object obj = handler.getValue("h");
            handler = SystemMetaObject.forObject(obj);
        }
        while (handler.hasGetter("target")) {
            Object obj = handler.getValue("target");
            handler = SystemMetaObject.forObject(obj);
        }
        //获取连接对象
        ...
        MappedStatement mappedStatement =
                (MappedStatement) handler.getValue("delegate.mappedStatement");
        ...
        if (mapId.matches(".+GaoPage$")) {
            ...
            ParameterHandler parameterHandler =
                    (ParameterHandler) handler.getValue("delegate.parameterHandler");
            //获取请求时的参数
            ...
            String sql = (String) handler.getValue("delegate.boundSql.sql");
            ...
            handler.setValue("delegate.boundSql.sql", limitSql);
        }
        //调用原对象的方法，进入责任链的下一级
